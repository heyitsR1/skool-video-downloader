...
// ffmpeg.wasm's worker posts failures back as `e.toString()` — a plain string,
// not an Error — so anything rejected out of exec/writeFile/readFile has no
// `.message`. Reading it blind yields undefined and the service worker falls
// back to a bare "Merge failed" with the real reason (OOM, abort, muxer error)
// thrown away. Normalize the value, and attach the ffmpeg log tail for merges.
function errText(err, withTail = false) {
  const base = (err && err.message) || (typeof err === 'string' ? err : '') || String(err) || 'Unknown error';
  const tail = withTail ? logTail() : '';
  return tail ? `${base}\n${tail}` : base;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'MERGE_PING') {
    ...
    handleMerge(message)
      ...
      .catch((err) => sendResponse({ success: false, error: errText(err, true) }));
    return true; // async
  } else if (message.type === 'CREATE_BLOB_URL') {
    createBlobUrl(message)
      .then((result) => sendResponse({ success: true, ...result }))
      .catch((err) => sendResponse({ success: false, error: errText(err) }));
    return true; // async
  } else if (message.type === 'SAVE_CLICK') {
    // The FALLBACK save path. The worker's own chrome.downloads.download of the
    // blob URL is preferred and tried first (see saveViaOffscreenAnchor); this
    // runs only if that failed.
    //
    // It cannot put a file in a subfolder: the HTML `download` attribute cannot
    // hold a directory, so an anchor flattens "Course/01 Lesson.mp4" into
    // "Course_01 Lesson.mp4" and drops it beside the folder — which is why it is
    // the fallback and not the main path. It also cannot report the download id,
    // leaving the worker to find its own download afterwards.
    //
    // chrome.downloads does not exist in an offscreen document, so this handler
    // could not call it even if that were wanted.
    try {
      if (!currentJob || !currentJob.objectUrl) throw new Error('No file ready to save');
      const a = document.createElement('a');
      a.href = currentJob.objectUrl;
      // The attribute cannot hold a directory; the worker's own download path is
      // what puts a course backup in its folder. This is the fallback.
      a.download = String(message.filename || 'download').split('/').pop();
      document.body.appendChild(a);
      a.click();
      a.remove();
      sendResponse({ success: true });
    } catch (err) {
      sendResponse({ success: false, error: errText(err) });
    }
  } else if (message.type === 'GET_DOWNLOAD_URL') {
    // The blob URL for whatever is currently staged. chrome.downloads is not
    // available in an offscreen document, but the worker can download a URL
    // minted here — and only the worker can name a subfolder.
    sendResponse(currentJob && currentJob.objectUrl
      ? { success: true, downloadUrl: currentJob.objectUrl }
      : { success: false, error: 'No file ready to save' });
...
async function handleMerge(message) {
  ...
  // ffmpeg can exit 0 having written nothing usable. Saving that produces a file
  // the user's player opens as a 00:00:00 track with no picture, reported as a
  // success by everything upstream. Fail here instead.
  if (blob.size < 64 * 1024) {
    throw new Error(`Merge produced an empty file (${blob.size} bytes)\n${logTail()}`);
  }
  job.objectUrl = URL.createObjectURL(blob);
  return { downloadUrl: job.objectUrl, size: blob.size };
}
