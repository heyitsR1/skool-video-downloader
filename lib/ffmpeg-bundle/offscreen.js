...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'MERGE_PING') {
    ...
    // The save has to start from inside this document. A blob: URL minted here
    ...
    // to, so Chrome rejects it with NETWORK_INVALID_REQUEST.
    //
    // Calling downloads.download from here, rather than clicking an anchor, is
    // what lets a course backup put the file in its course folder: the HTML
    // `download` attribute cannot hold a directory, so an anchor flattens
    // "Course/01 Lesson.mp4" into "Course_01 Lesson.mp4" and drops it beside the
    // folder. It also returns the id directly, instead of leaving the worker to
    // find its own download again afterwards. The anchor stays as a fallback.
    try {
