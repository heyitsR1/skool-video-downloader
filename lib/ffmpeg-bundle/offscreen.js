...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'MERGE_PING') {
    ...
    saveCurrentJob(message.filename)
...
async function saveCurrentJob(filename) {
    ...
    try {
      const downloadId = await new Promise((resolve, reject) => {
        chrome.downloads.download(
          { url: currentJob.objectUrl, filename: name, conflictAction: 'overwrite', saveAs: false },
          (id) => chrome.runtime.lastError ? reject(new Error(chrome.runtime.lastError.message)) : resolve(id));
      });
      return { downloadId };
    } catch (err) {
      // Fall through to the anchor: a save that lands in the wrong folder still
      // beats no save at all, and the worker verifies where it went either way.
      lastSaveFallback = errText(err);
    }
  }
  ...
  a.click();
  a.remove();
  return { downloadId: null, fallback: lastSaveFallback || 'downloads API unavailable' };
}
let lastSaveFallback = null;
